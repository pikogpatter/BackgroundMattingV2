...
parser.add_argument(
    "--backend", type=str, default="pytorch", choices=["pytorch", "torchscript"]
)
...
assert type(args.image_src) == type(
    args.image_bgr
), "Image source and background must be provided together."
assert (not args.image_src) != (
    not args.resolution
), "Must provide either a resolution or an image and not both."
...
if not args.image_src:
    src = torch.rand(
        (args.batch_size, 3, *args.resolution[::-1]), device=device, dtype=precision
    )
    bgr = torch.rand(
        (args.batch_size, 3, *args.resolution[::-1]), device=device, dtype=precision
    )
    ...
    src = (
        to_tensor(Image.open(args.image_src))
        .unsqueeze(0)
        .repeat(args.batch_size, 1, 1, 1)
        .to(device=device, dtype=precision)
    )
    bgr = (
        to_tensor(Image.open(args.image_bgr))
        .unsqueeze(0)
        .repeat(args.batch_size, 1, 1, 1)
        .to(device=device, dtype=precision)
    )
