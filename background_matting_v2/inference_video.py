...
parser.add_argument(
    "--output-format", type=str, default="video", choices=["video", "image_sequences"]
)

args = parser.parse_args()


assert "err" not in args.output_types or args.model_type in [
    "mattingbase",
    "mattingrefine",
], "Only mattingbase and mattingrefine support err output"
assert "ref" not in args.output_types or args.model_type in [
    "mattingrefine"
...
class VideoWriter:
    def __init__(self, path, frame_rate, width, height):
        self.out = cv2.VideoWriter(
            path, cv2.VideoWriter_fourcc(*"mp4v"), frame_rate, (width, height)
        )
...
class ImageSequenceWriter:
    ...
    def _add_batch(self, frames, index):
        ...
        for i in range(frames.shape[0]):
            ...
            frame.save(
                os.path.join(self.path, str(index + i).zfill(5) + "." + self.extension)
            )
...
if os.path.exists(args.output_dir):
    if (
        input(f"Directory {args.output_dir} already exists. Override? [Y/N]: ").lower()
        == "y"
    ):
...
if args.output_format == "video":
    ...
    if "com" in args.output_types:
        com_writer = VideoWriter(
            os.path.join(args.output_dir, "com.mp4"), vid.frame_rate, w, h
        )
    if "pha" in args.output_types:
        pha_writer = VideoWriter(
            os.path.join(args.output_dir, "pha.mp4"), vid.frame_rate, w, h
        )
    if "fgr" in args.output_types:
        fgr_writer = VideoWriter(
            os.path.join(args.output_dir, "fgr.mp4"), vid.frame_rate, w, h
        )
    if "err" in args.output_types:
        err_writer = VideoWriter(
            os.path.join(args.output_dir, "err.mp4"), vid.frame_rate, w, h
        )
    if "ref" in args.output_types:
        ref_writer = VideoWriter(
            os.path.join(args.output_dir, "ref.mp4"), vid.frame_rate, w, h
        )
else:
    if "com" in args.output_types:
    ...
    for src, bgr in tqdm(DataLoader(dataset, batch_size=1, pin_memory=True)):
        ...
        if "com" in args.output_types:
            if args.output_format == "video":
                ...
                bgr_green = torch.tensor(
                    [120 / 255, 255 / 255, 155 / 255], device=device
                ).view(1, 3, 1, 1)
        ...
        if "err" in args.output_types:
            err_writer.add_batch(
                F.interpolate(err, src.shape[2:], mode="bilinear", align_corners=False)
            )
        if "ref" in args.output_types:
